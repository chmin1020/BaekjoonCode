		...
		for (int j = 1; j <= size; j++) {
			if (!visited[i][j]) {
				...
				do {
					...
					if (tpx > 1 && !visited[tpx - 1][tpy] && pic[tpx - 1][tpy] == bfs.front().second) {
						bfs.push(make_pair(make_pair(tpx - 1, tpy), pic[tpx - 1][tpy]));
						visited[tpx - 1][tpy] = true;
					}
					if (tpx < size && !visited[tpx + 1][tpy] && pic[tpx + 1][tpy] == bfs.front().second) {
						bfs.push(make_pair(make_pair(tpx + 1, tpy), pic[tpx + 1][tpy]));
						visited[tpx + 1][tpy] = true;
					}
					if (tpy > 1 && !visited[tpx][tpy - 1] && pic[tpx][tpy - 1] == bfs.front().second) {
						bfs.push(make_pair(make_pair(tpx, tpy - 1), pic[tpx][tpy - 1]));
						visited[tpx][tpy - 1] = true;
					}
					if (tpy < size && !visited[tpx][tpy + 1] && pic[tpx][tpy + 1] == bfs.front().second) {
						bfs.push(make_pair(make_pair(tpx, tpy + 1), pic[tpx][tpy + 1]));
						visited[tpx][tpy + 1] = true;
					}
					bfs.pop();
