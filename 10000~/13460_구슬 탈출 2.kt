...
fun simulation(board: Array<CharArray>, gameStat: Game, cnt: Int) {
    ...
    for (it in 0 until DIRECT_CNT) {
        ...
        if (newBlue == newRed && newBlue != gameStat.hole) {
            if (whatWasFollowed(gameStat, it) == 'R')
                newRed = Pos(newRed.x - directX[it], newRed.y - directY[it])
            else
                newBlue = Pos(newBlue.x - directX[it], newBlue.y - directY[it])
        }

        //파랑 구슬이 빠짐 -> 더 볼 필요 x
        if (newBlue == gameStat.hole)
            continue
        ...
        //카피 보드판 생성
        val copyBoard = Array(board.size) { CharArray(board[0].size) }
        for (i in board.indices)
            for (j in board[0].indices)
                copyBoard[i][j] = board[i][j]

        //이동 적용
        copyBoard[gameStat.red.x][gameStat.red.y] = '.'
        copyBoard[newRed.x][newRed.y] = 'R'
        copyBoard[gameStat.blue.x][gameStat.blue.y] = '.'
        copyBoard[newBlue.x][newBlue.y] = 'B'

        //재귀로 다음 시뮬레이션
        simulation(copyBoard, Game(newRed, newBlue, gameStat.hole), cnt + 1)
    }
}
