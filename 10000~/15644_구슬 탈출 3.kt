...
val directCh = charArrayOf('U', 'D', 'L', 'R')

data class Pos(val x: Int = -1, val y: Int = -1)
data class Game(val red: Pos, val blue: Pos, val hole: Pos, val route: String)
...
fun main() {
    ...
    if(answer != Int.MAX_VALUE) println(answerRoute)
}
...
fun simulation(board: Array<CharArray>, gameStat: Game, cnt: Int) {
    ...
    for (it in 0 until DIRECT_CNT) {
        ...
        val newRoute = gameStat.route + directCh[it]
        ...
        //재귀로 다음 시뮬레이션
        simulation(board, Game(newRed, newBlue, gameStat.hole, newRoute), cnt + 1)
    }
}
